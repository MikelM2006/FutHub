...
export function MiPerfilModal({
    ...
    if (profile && profile.id) {
      ...
      if (rawPos) {
        ...
        if (matchEnum) {
          ...
          const matchLabel = POSICIONES.find(
            (p) => p.label.toLowerCase() === String(rawPos).toLowerCase(),
          );
    ...
    try {
      if (Object.keys(toUpdate).length > 0 && id) {
        ...
        setEditedProfile((prev) => ({
          ...prev,
          posicion: respJson.posicion || updated.posicion,
        }));
      }
  ...
  const handleEliminar = async () => {
    ...
    try {
      const url = profile?.id
        ? `${apiBase.replace(/\/$/, "")}/api/usuarios/${profile.id}`
        : `${apiBase.replace(/\/$/, "")}/api/usuarios/`;
      const res = await fetch(url, {
        method: "DELETE",
        headers: {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="w-[96vw] sm:w-[94vw] md:w-[92vw] lg:w-[88vw] xl:w-[82vw] max-w-[1600px] bg-white max-h-[90vh] overflow-y-auto px-12 py-8"
        ...
        <div className="w-full flex-1 overflow-y-auto pb-4">
          ...
          >
            <TabsList className="grid w-full grid-cols-2 gap-4 bg-gray-100 p-2 rounded-md">
              <TabsTrigger
                value="informacion"
                className="data-[state=active]:bg-white data-[state=active]:text-[#16a34a] py-3"
              >
                Información
              </TabsTrigger>
              <TabsTrigger
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <p className="text-[#111827] text-lg w-full">
                      {profile?.nombreCompleto || editedProfile.nombreCompleto}
                    </p>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="text-[#111827] text-lg w-full">
                      {displayPosition(
                        editedProfile.posicion || profile?.posicion,
                      )}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="mt-2">
                    {renderStaticStars(
                      profile?.autoclasificacion ??
                        editedProfile.autoclasificacion,
                    )}
                  </div>
                </div>
            ...
            <TabsContent value="editar" className="space-y-6 mt-6 p-6">
              <div className="space-y-6">
                <div className="grid grid-cols-1 gap-6">
                  <div className="space-y-3">
                    <Label htmlFor="edit-posicion" className="text-[#111827]">
                      Posición
                    </Label>
                    <select
                      id="edit-posicion"
                  ...
                  <div className="space-y-3">
                    <Label htmlFor="edit-clave" className="text-[#111827]">
                      Nueva contraseña
                    </Label>
                    <Input
                      id="edit-clave"
                  ...
                  <Button
                    ...
                    className="bg-[#16a34a] hover:bg-[#15803d] text-white w-full mt-4"
                  >
                    Guardar Cambios
                  </Button>
                </div>
            ...
            <div className="w-full sm:w-auto flex-1">
              ...
              {!profile?.id && (
                <p className="mt-2 text-sm text-gray-500 text-center">
                  Nota: No se detectó un id de usuario en este contexto; al
                  eliminar no se modificará el backend.
                </p>
              )}
            </div>
          )}
