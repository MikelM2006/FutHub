...
async function handleResponse(response) {
    if (!response.ok) {
        ...
        const errorMessage = data.message || 'Ocurrió un error inesperado en el servidor';
                ...
                <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                            <h3 className="truncate font-semibold text-[#111827]">{nombre}</h3>
                            {isOwner && (
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-md bg-white">
                ...
                <DialogFooter>
                    ...
                    <Button onClick={handleCrear} disabled={!nombreEquipo.trim() || isCreating} className="bg-[#16a34a] hover:bg-[#15803d]">
...
function ConfirmarUnirseModal({ open, onOpenChange, nombreEquipo, onConfirmar }) {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-md bg-white">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        ¿Estás seguro de que quieres unirte a <span className="text-[#16a34a] font-bold">{nombreEquipo}</span>?
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter className="mt-4">
                    <Button variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button>
                    <Button onClick={handleConfirmar} disabled={isJoining} className="bg-[#16a34a] hover:bg-[#15803d]">
...
export function Equipos({ userName = 'Usuario', user, onNavigate, onLogout }) {
    ...
    const [selectedEquipo, setSelectedEquipo] = useState({ id: null, nombre: '', miembros: [], isOwner: false });
    ...
    const cargarDatos = async () => {
        setIsLoading(true);
        try {
            const [resMisEquipos, resTodosEquipos, resInvitaciones] = await Promise.all([
                fetch(`${BASE_URL}/equipos/mis-equipos`, { headers: getHeaders(false) }).then(handleResponse),
                fetch(`${BASE_URL}/equipos/buscar`, { headers: getHeaders(false) }).then(handleResponse),
                fetch(`${BASE_URL}/equipos/invitaciones`, { headers: getHeaders(false) }).then(handleResponse),
            ]);
    ...
    const handleVerMiembrosBuscar = async (equipo) => {
        try {
            const detalle = await fetch(`${BASE_URL}/equipos/${equipo.id}`, { headers: getHeaders(false) }).then(handleResponse);
            setSelectedEquipo({
                id: detalle.id,
    ...
    const handleResponderInvitacion = async (invitacionId, aceptar) => {
        try {
            await fetch(`${BASE_URL}/equipos/invitaciones/${invitacionId}/responder`, {
                method: 'POST',
                headers: getHeaders(true),
    ...
    const handleInviteMember = async (email) => {
        ...
        if (!email || !emailRegex.test(email)) {
            alert("El formato del correo es inválido. Verifica que tenga '@' y un dominio.");
            return;
        }
        ...
        try {
            ...
            // Recargar datos
            const detalle = await fetch(`${BASE_URL}/equipos/${selectedEquipo.id}`, { headers: getHeaders(false) }).then(handleResponse);
            setSelectedEquipo(prev => ({ ...prev, miembros: detalle.miembros || [] }));
            await cargarDatos();
            ...
            } else if (errorMsg.includes("invitación pendiente") || errorMsg.includes("invitacion pendiente")) {
                alert(" Ya le has enviado una invitación a este usuario anteriormente.");
    ...
    const handleLeaveTeam = async () => {
        if (!selectedEquipo?.id) return;
        if (!window.confirm(`¿Seguro que quieres salir de "${selectedEquipo.nombre}"?`)) return;
    ...
    return (
        <DashboardLayout userName={userName} user={user} onNavigate={onNavigate} onLogout={onLogout} currentPage="equipos">
            <div className="space-y-6">
                ...
                <Tabs defaultValue="mis-equipos" className="w-full">
                    <TabsList className="bg-[#f3f4f6] p-1 rounded-lg inline-flex mb-6">
                        <TabsTrigger value="mis-equipos" className="rounded-md px-6 py-2 text-sm font-medium transition-all data-[state=active]:bg-white text-[#6b7280]">
                            Mis Equipos
                        </TabsTrigger>
                        <TabsTrigger value="buscar-equipos" className="rounded-md px-6 py-2 text-sm font-medium transition-all data-[state=active]:bg-white text-[#6b7280]">
                            Buscar Equipos
                        </TabsTrigger>
                        <TabsTrigger value="invitaciones" className="rounded-md px-6 py-2 text-sm font-medium transition-all data-[state=active]:bg-white text-[#6b7280] flex items-center gap-2">
                            Invitaciones
                    ...
                    <TabsContent value="mis-equipos" className="space-y-6">
                        <Button className="bg-[#16a34a] hover:bg-[#15803d] text-white" onClick={() => setIsCrearModalOpen(true)}>
                            <Plus className="w-5 h-5 mr-2" /> Crear Equipo
                        </Button>
                        <div className="space-y-4">
                            {misEquipos.length === 0 ? <p className="text-gray-500 italic">No tienes equipos.</p> :
                                misEquipos.map((equipo) => (
                                    <EquipoCard key={equipo.id} nombre={equipo.nombre} miembros={equipo.miembros?.length || 0} isOwner={equipo.isOwner} onClick={() => handleVerMiembros(equipo)} />
                                ))
                            }
                        </div>
                    </TabsContent>
                    ...
                    <TabsContent value="buscar-equipos" className="space-y-6">
                        <div className="relative">
                            ...
                            <Input type="text" placeholder="Buscar equipos..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
                        </div>
                        <div className="space-y-4">
                            {equiposFiltrados.map((equipo) => (
                                <EquipoCard key={equipo.id} nombre={equipo.nombre} miembros={equipo.miembros} isOwner={equipo.isOwner} onJoin={() => handleJoinTeam(equipo)} onClick={() => handleVerMiembrosBuscar(equipo)} />
                            ))}
                        </div>
                    </TabsContent>
                    ...
                    <TabsContent value="invitaciones" className="space-y-6">
                        <div className="space-y-4">
                            {invitaciones.length === 0 ? <p className="text-center py-10 text-gray-500 italic">No tienes invitaciones.</p> :
                                invitaciones.map((invitacion) => (
                                    <InvitacionCard key={invitacion.id} invitacion={invitacion} onAceptar={(id) => handleResponderInvitacion(id, true)} onRechazar={(id) => handleResponderInvitacion(id, false)} />
                                ))
                            }
                        </div>
                    </TabsContent>
            ...
            <CrearEquipoModal open={isCrearModalOpen} onOpenChange={setIsCrearModalOpen} onCrearEquipo={handleCrearEquipo} />
            <ConfirmarUnirseModal open={isConfirmarModalOpen} onOpenChange={setIsConfirmarModalOpen} nombreEquipo={selectedEquipo.nombre} onConfirmar={handleConfirmarUnirse} />
            <VerMiembrosModal open={isMiembrosModalOpen} onOpenChange={setIsMiembrosModalOpen} nombreEquipo={selectedEquipo.nombre} miembros={selectedEquipo.miembros} isOwner={selectedEquipo.isOwner} onInvite={handleInviteMember} onDelete={handleDeleteTeam} onLeave={handleLeaveTeam} />
        </DashboardLayout>
    );
}