...
const API_URL = import.meta.env.VITE_API_URL + "/api/partidos";
  ...
  if (!response.ok) {
    const message =
      parsedBody && parsedBody.message
        ? parsedBody.message
        : bodyText ||
          `Error en la solicitud a la API (status ${response.status})`;
...
export function Partidos({ userName = "Usuario", user, onNavigate, onLogout }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const mensajeFechasPasadas =
        "No se pueden crear partidos en fechas pasadas.";
      const mensajeCanchaReservada =
        "La cancha ya esta reservada en la fecha seleccionada.";

      if (
        String(bodyText).includes(mensajeFechasPasadas) ||
        String(bodyText).includes(mensajeCanchaReservada)
      ) {
          ...
          <div className="md:col-span-1">
            ...
            <Card className="p-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  onValueChange={(value) => setSelectedEquipoA(value)}
                  value={selectedEquipoA}
                >
                  <SelectTrigger
                    id="equipo-a"
        ...
        {currentStep === 4 && (
          <div className="md:col-span-1">
            ...
            <Card className="p-6 space-y-4">
              ...
              {selectedCancha &&
                (() => {
                  const canchaObj = canchas.find(
                    (c) => c.id === selectedCancha,
                  );
                  return (
                    <div>
                      <Label className="text-[#6b7280]">Cancha</Label>
                      <p className="text-[#111827]">
                        {canchaObj ? canchaObj.nombre : selectedCancha}
                      </p>
                      <p className="text-sm text-[#6b7280]">
                        {canchaObj ? canchaObj.ubicacion : ""}
                      </p>
                    </div>
                  );
                })()}
              ...
              <div>
                ...
                <p className="text-[#111827]">
                  {selectedEquipoA
                    ? misEquipos.find((e) => e.id === selectedEquipoA)
                        ?.nombre || selectedEquipoA
                    : "No seleccionada"}
                </p>
              </div>
              <div>
